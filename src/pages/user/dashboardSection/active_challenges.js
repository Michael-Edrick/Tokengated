...
function ActiveChallenges({
  ...
  // const [gameName, setGameName] = useState(activeGameDetail?.name);
  const [gameId, setGameId] = useState(activeGameDetail?.id);
  // const [amount, setAmount] = useState(activeGameDetail?.totalContribution);
  const [user, setUser] = useLocalStorage("user", null)
  // const [link, setLink] = useState(
  //   `${SHARE_LINK_URL}user/join_game?gameId=${activeGameDetail?.id}`
  // );
  // const [copyMessage, setCopyMessage] = useState("");
  // const handleCreateGame = async () => {
  //   router.push("/user/guess_winner?gameId=" + activeGameDetail?.id);
  // };
  // const [isPlayerJoin, setIsPlayerJoin] = useState(false);
  ...
  // useEffect(() => {
  //   if (activeGameDetail?.numberOfPlayers === activeGameDetail?.team.length) {
          ...
          <div className=" justify-center md:flex md:justify-center pb-4 font-archivo" style={{ fontFamily: "'Archivo', sans-serif" }}>
            <div className="min-h-[350px] sm:min-w-[350px] bg-black shadow-md rounded px-1 py-1 flex flex-col justify-between items-center mt-6 space-y-6 relative">
              ...
              <div className="w-full flex justify-around items-center px-2 mt-2 border border-gray-300 rounded py-2">
                ...
                <div
                  className={`${activeGameDetail?.status === "finished"
                ...
                <Button
                  ...
                  className={`bg-[#111922] px-0 py-0 ${activeGameDetail?.status === "voting" || activeGameDetail?.status === "finished"
